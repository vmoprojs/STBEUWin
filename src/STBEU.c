                ...
                if(l==m)
                {
                    for(v=t+1;v<*ntime;v++)
                    {
                        ...
                        if(lagt<=maxtime[0])
                        {
                            ...
                            Grad_Pair_Gauss(rho,flagnuis,gradcor,grad,npar,nuis,sdata[(t+*ntime*l)],sdata[(v+*ntime*l)],cormod,0,lagt,flagcor,nparcT,parcor);
                            /*if(weigthed[0])
                             {
                ...
                else
                {
                    ...
                    for(v=0;v<*ntime;v++)
                    {
                        ...
                        if(lagt<=maxtime[0] && lags<=maxdist[0])
                        {
                            ...
                            //Compute the gradient of the composite likelihood:
                            Grad_Pair_Gauss(rho,flagnuis,gradcor,grad,npar,nuis,sdata[(t+*ntime*l)],sdata[(v+*ntime*m)],cormod,lags,lagt,flagcor,nparcT,parcor);
                            /*if(weigthed[0])
                             {
...
void SubSamp_space(double *coordx, double *coordy,double *coordt, int *ncoord,int *ntime,int *cormod,double *data,int *dist, double *maxdist,double *maxtime,int *npar,double *parcor,int *nparc,double *nuis,int *nparnuis, int *flagcor, int *flagnuis,double *vari,double *winc, double *winstp,double *a, double *b,double *block_mean,int *weigthed, int *local_wi, int *dev)
{
    ...
    int *npts, numintx=0, numinty=0,*nparcT=0;
    int n_win=0,kk=0,h=0,i=0,nsub,j=0,p=0,q=0,r,nmat;
    
    nparcT=(int *) R_alloc(1, sizeof(int));
    //*nparcT = *npar+*nparc;
    
    if(cormod[0]==1) nparcT[0] = 2;
    if(cormod[0]==2) nparcT[0] = 5;
    if(cormod[0]==3) nparcT[0] = 7;
    ...
    //Rprintf("A. MIERDAAA\n");
   //Rprintf("A. npar: %d nparc: %d nparnuis: %d nparcT: %d\n",*npar,*nparc,*nparnuis,*nparcT);
    
   //Rprintf("A. nuis: %f nuis: %f nuis: %f\n",nuis[0],nuis[1],nuis[2]);
    
    //Rprintf("A. parcor: %f parcor: %f parcor: %f parcor: %f parcor: %f parcor: %f parcor: %f\n",parcor[0],parcor[1],parcor[2],parcor[3],parcor[4],parcor[5],parcor[6]);
        ...
        {  // cycle for each block window
            *npts=0;   // number of loc sites in the window
               ...
               (npts[0]>5))
            {
                ...
                scalar_space(npts,ntime,coordt,maxtime,maxdist,cormod,parcor,flagcor,flagnuis,npar,nuis,sdata,weigthed,mom_cond,dist, scoordx,scoordy,gradcor,grad,ww, nparcT);
                ...
                if(l==m)
                {
                    for(v=t+1;v<nstime[0];v++)
                    {
                        ...
                        if(lagt<=maxtime[0])
                        {
                            ...
                            //Compute the gradient of the composite likelihood:
                            Grad_Pair_Gauss(rho,flagnuis,gradcor,grad,npar,nuis,sdata[(t+nstime[0]*l)],sdata[(v+nstime[0]*l)],cormod,0,lagt,flagcor,nparcT,parcor);
                            /*if(*weigthed)
                             {
                             weights=CorFunBohman(lagt,maxtime[0]);
                             ww[0]=1;ww[1]=1;ww[2]=weights;ww[3]=weights;
                ...
                else
                {
                    ...
                    for(v=0;v<nstime[0];v++)
                    {
                        ...
                        if(lagt<=maxtime[0] && lags<=maxdist[0])
                        {
                            ...
                            GradCorrFct(rho,cormod,flagcor,gradcor,lags,lagt,parcor);
                            //Compute the gradient of the composite likelihood:
                            Grad_Pair_Gauss(rho,flagnuis,gradcor,grad,npar,nuis,sdata[(t+nstime[0]*l)],sdata[(v+nstime[0]*m)],cormod,lags,lagt,flagcor,nparcT,parcor);
                            /*if(*weigthed)
                             {
                             weights=CorFunBohman(lags,maxdist[0])*CorFunBohman(lagt,maxtime[0]);
                             ww[0]=1;ww[1]=1;ww[2]=weights;ww[3]=weights;
...
void SubSamp_time(double *coordx, double *coordy,double *coordt, int *ncoord,int *ntime,int *cormod,double *data,int *dist, double *maxdist,double *maxtime,int *npar,double *parcor,int *nparc,double *nuis,int *nparnuis, int *flagcor, int *flagnuis,double *vari,double *a, double *b,double *winc, double *winstp,double *block_mean,int *weigthed,int *local_wi, int *dev)
{
    ...
    int nsub=0, nstime=0;
    int kk=0,h=0,i=0,p=0,q=0,r,nmat, *nparcT=0;
    nparcT=(int *) R_alloc(1, sizeof(int));
    ...
    {//loop for the number of sub-sampling:
      ...
      // printf("C gradcor: %f\t%f\t%f\n\n",gradcor[0],gradcor[1],gradcor[2]);
        scalar_time(ncoord,&nstime,sublagt,cormod,parcor,flagcor,gradcor,flagnuis,grad,npar,nuis,sdata,weigthed,maxtime,ww,mom_cond,dist,coordx,coordy,maxdist,nparcT);
                ...
                if(l==m)
                {
                    for(v=t+1;v<nstime[0];v++)
                    {
                        ...
                        if(lagt<=maxtime[0])
                        {
                            ...
                            GradCorrFct(rho,cormod,flagcor,gradcor,0,lagt,parcor);
                            //Compute the gradient of the composite likelihood:
                            Grad_Pair_Gauss(rho,flagnuis,gradcor,grad,npar,nuis,s2data[(t+nstime[0]*l)],s2data[(v+nstime[0]*l)],cormod,0,lagt,flagcor,nparcT,parcor);
                            /*if(*weigthed)
                             {
                             weights=CorFunBohman(lagt,maxtime[0]);
                             ww[0]=1;ww[1]=1;ww[2]=weights;ww[3]=weights;
                ...
                else
                {
                    ...
                    for(v=0;v<nstime[0];v++)
                    {
                        ...
                        if(lagt<=maxtime[0] && lags<=maxdist[0])
                        {
                            ...
                            GradCorrFct(rho,cormod,flagcor,gradcor,lags,lagt,parcor);
                            //Compute the gradient of the composite likelihood:
                            Grad_Pair_Gauss(rho,flagnuis,gradcor,grad,npar,nuis,s2data[(t+nstime[0]*l)],s2data[(v+nstime[0]*m)],cormod,lags,lagt,flagcor,nparcT,parcor);
                            /*if(*weigthed)
                             {
                             weights=CorFunBohman(lags,maxdist[0])*CorFunBohman(lagt,maxtime[0]);
                             ww[0]=1;ww[1]=1;ww[2]=weights;ww[3]=weights;
...

{
    ...
    int n_win=0,kk=0,h=0,i=0,nsub,nsub_t=0,j=0,f=0,p=0,q=0,r,nmat,*nparcT=0;
            ...
            *npts=0;   // number of points in the block
               ...
               (npts[0]>5))
            {
                ...
                {//loop for the number of sub-sampling:
                    ...
                    scalar_spacetime(npts,&nstime,sublagt,maxtime,cormod,parcor,flagcor,gradcor,flagnuis,grad,npar,nuis,s2data,weigthed,ww,mom_cond,maxdist,dist,scoordx,scoordy,nparcT);
